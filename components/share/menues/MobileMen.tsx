...
const MobileMenu: NextPage = () => {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button
          className="inline-flex w-full justify-center rounded-md  px-2 py-2 text-sm font-medium text-primary border-primary border-2 focus:outline-none"
            ...
            {HeaderMenuRo.map((item, index) => (
              ...
              {({ active }) => (
                <Link
                  href={item.url}
                  className={`${
                    router.pathname === item.url ? "text-orange" : "text-primary"
